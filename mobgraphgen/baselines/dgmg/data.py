...
class DGMG_Dataset_from_file(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #print("in baselines/data and graph.nodes= ",graph.nodes(data=True))
        #print("in baselines/data and graph.nodes[v]['label'] = ",graph.nodes[(0,0)]['label'])

        actions = []
        for v in range(len(graph.nodes())):
            #print("in baselines/data and v= ",v,"len(graph.nodes) = ",len(graph.nodes()))
            #print("in baselines/data and graph.nodes[v]['label'] = ",graph.nodes[v]['label'])
