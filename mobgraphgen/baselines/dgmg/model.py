...
class GraphEmbed(nn.Module):
    ...
    def forward(self, g_list):
        ...
        #added by Behnaz in 2022
        #bg=bg.to(torch.device('cuda:0' if torch.cuda.is_available() else 'cpu'))
...
class AddNode(nn.Module):
    ...
    def _initialize_node_repr(self, g, node_type, graph_embed):
        ...
        g.nodes[num_nodes - 1].data['label'] = torch.LongTensor([[node_type]]).to(device=torch.device('cuda:0' if torch.cuda.is_available() else 'cpu'))
        

        #added by Behnaz in 2022
        #g.to(torch.device('cuda:0' if torch.cuda.is_available() else 'cpu'))
...
class ChooseDestAndUpdate(nn.Module):
    ...
    def _initialize_edge_repr(self, g, src_list, dest_list, edge_types):
        ...
        g.edges[src_list, dest_list].data['label'] = (torch.LongTensor(edge_types).unsqueeze(1)).to(device=torch.device('cuda:0' if torch.cuda.is_available() else 'cpu'))
...
class DGMG(nn.Module):
    def __init__(self, v_max, node_hidden_size, num_prop_rounds,
        ...
        self.device = device
        #added by Behnaz
        print("in baselines/dgmg/mode.py and in init of DGMG and self.device = ",self.device)
    ...
    def prepare(self, batch_size, training):
        ...
        for i in range(batch_size):
            ...
            #added by Behnaz on 25.4.22
            #g = g.to('cuda:0' if torch.cuda.is_available() else 'cpu')
            #g = g.to('cuda:0')
            #g = g.to(torch.device('cuda:0'))
            #g = g.to(torch.device('cuda:0'))
            g = g.to(torch.device('cuda:0' if torch.cuda.is_available() else 'cpu'))
