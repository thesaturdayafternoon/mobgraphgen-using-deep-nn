...
#addded by Beehnaz on 29.5.22
def get_subfolder_number(idx,subdir_size):
  return (math.ceil((idx+1)/float(subdir_size)))-1


#addded by Beehnaz on 29.5.22
def create_sub_dirs(parent_dir,subdirs_numb):
  for i in range(0,subdirs_numb):
    os.mkdir(os.path.join(parent_dir, str(i)+"/"))

#addded by Beehnaz on 29.5.22
def mk_newpath_from_oldpath(oldpath,oldparent,newparent):
  new_path=newparent +  oldpath[len(oldparent):]
  return new_path

#addded by Beehnaz on 29.5.22
def create_sub_dir(parent_dir,subdirs_numb):
  current_dataset_path=os.path.join(parent_dir, str(subdirs_numb)+"/")
  os.mkdir(current_dataset_path)
  subdirs_numb+=1
  return current_dataset_path,subdirs_numb



#addded by Beehnaz on 29.5.22
def listdir_from_subdirs(parentdir):
  return [os.path.join(path, file) for path, subdirs, files in os.walk(parentdir) for file in files]





#addded by Beehnaz on 3.6.22
def save_graphs_dataset(graphs,args,subdir_size):
  if len(graphs)>subdir_size:
    subdirs_numb=math.ceil(len(graphs)/float(subdir_size))
    create_sub_dirs(args.current_dataset_path,subdirs_numb)
    for count in range(0,len(graphs)):
      #print("count = ", count, " = ",str(get_subfolder_number(int(count),subdir_size)))
      #with open(os.path.join(current_dataset_path, 'graph{}.dat'.format(count)), 'wb') as f:
      with open(os.path.join(args.current_dataset_path, str(get_subfolder_number(int(count),subdir_size))+"/") + 'graph' + str(count) + '.dat', 'wb') as f:
        pickle.dump(graphs[count], f)
  else:
    for count in range(0,len(graphs)):
      #print("count = ", count, " = ",str(get_subfolder_number(int(count),subdir_size)))
      with open(os.path.join(args.current_dataset_path, 'graph{}.dat'.format(count)), 'wb') as f:
      #with open(os.path.join(args.current_dataset_path, str(get_subfolder_number(int(count),subdir_size))+"/") + 'graph' + str(count) + '.dat', 'wb') as f:
        pickle.dump(graphs[count], f)
...
def mapping(path, dest):
    ...
    #added by Behnaz
    for filename in tqdm(listdir_from_subdirs(path)):
    #for filename in tqdm(os.listdir(path)):
        if filename.endswith(".dat"):
            ...
            for _, data in G.nodes.data():
                ...
                #if data.get('label','') not in node_forward:
                    node_forward[data['label']] = node_count
                    #node_forward[data.get('label','')] = node_count
                    node_backward[node_count] = data['label']
                    #node_backward[node_count] = data.get('label','')
                    node_count += 1
            ...
            for _, _, data in G.edges.data():
                ...
                #if data.get('label','') not in edge_forward:
                    edge_forward[data['label']] = edge_count
                    #edge_forward[data.get('label','')] = edge_count
                    edge_backward[edge_count] = data['label']
                    #edge_backward[edge_count] = data.get('label','') 
                    edge_count += 1
...
def graph_to_min_dfscode(graph_file, graphs_path, min_dfscodes_path, temp_path):
    #print("in preprocess/graph_to_min_dfscode and graph_file = ",graph_file)
    #added by Behnaz
    ...
    with open(graph_file, 'rb') as f:
        ...
        if len(G.edges()) == len(min_dfscode):
            ...
            #new_path=min_dfscodes_path +  graph_file[len(graphs_path):]
            #new_path=mk_newpath_from_oldpath(graph_file,graphs_path,min_dfscodes_path)
            with open(mk_newpath_from_oldpath(graph_file,graphs_path,min_dfscodes_path), 'wb') as f:
...
def graphs_to_min_dfscodes(graphs_path, min_dfscodes_path, temp_path):
    ...
    #print("in preprocess/graphs_to_min_dfscodes and graphs_path = ",graphs_path)
    
    graphs = []
    #the line below is added by Behnaz
    for filename in listdir_from_subdirs(graphs_path):
...
def dfscode_from_file_to_tensor_to_file(min_dfscode_file, min_dfscodes_path, min_dfscode_tensors_path, feature_map):
    ...
    with open(min_dfscode_file, 'rb') as f:
    ...
    #added by Behnaz
    #new_path=mk_newpath_from_oldpath(min_dfscode_file,min_dfscodes_path,min_dfscode_tensors_path)
    #with open(min_dfscode_tensors_path + min_dfscode_file, 'wb') as f:
    with open(mk_newpath_from_oldpath(min_dfscode_file,min_dfscodes_path,min_dfscode_tensors_path), 'wb') as f:
...
def min_dfscodes_to_tensors(min_dfscodes_path, min_dfscode_tensors_path, feature_map):
    ...
    for filename in listdir_from_subdirs(min_dfscodes_path):
        if filename.endswith(".dat"):
