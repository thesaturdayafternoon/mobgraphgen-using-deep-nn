...
def train(args, dataloader_train, model, feature_map, dataloader_validate=None):
    ...
    print("in train/train and args.lr = ",args.lr)
    print("in train/train and args.dropout = ",args.dropout)
    
    optimizer = {}

    for name, net in model.items():
        optimizer['optimizer_' + name] = optim.AdamW(
    ...
    while epoch < args.epochs:
        loss = train_epoch(epoch, args, model, dataloader_train, optimizer, scheduler, feature_map, writer)
        epoch += 1
        gc.collect()
        # logging
        if args.log_tensorboard:
        ...
        if epoch % args.epochs_print_loss == 0:
          print('Epoch: {}/{}, train loss: {:.6f}, validation loss: {:.6f}'.format(epoch, args.epochs, loss,loss_validate))
        ...
        if args.save_model and epoch != 0 and epoch % args.epochs_save == 0:
            ...
            print('Model Saved - Epoch: {}/{}, train loss: {:.6f}, validation loss: {:.6f}'.format(epoch, args.epochs, loss,loss_validate))
