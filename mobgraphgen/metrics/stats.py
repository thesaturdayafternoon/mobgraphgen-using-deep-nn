...
MAX_WORKERS = 48
#added by Behnaz
from datasets.preprocess import get_subfolder_number
...
def write_graphs_from_dir(graphs_path, graphs_indices, outfile):
    ...
    print("in metrics.stats.write_graphs_from_dir-1")
    graphs_indices_act = []
    with open(outfile, 'w') as fr:
        for ind in graphs_indices:
            #print("in metrics.stats.write_graphs_from_dir-2")
    ...
    print("in metrics.stats.write_graphs_from_dir-3")
    return graphs_indices_act


def write_graphs_from_dir_with_subdirs(graphs_path, graphs_indices, outfile,len_graphs,subdir_size):
#def write_graphs_from_dir(graphs_path, graphs_indices, outfile):
...
def novelity(graph_ref_path, graph_ref_indices, graph_pred_path, graph_pred_indices, temp_path,len_graphs,subdir_size ,timeout):
#def novelity(graph_ref_path, graph_ref_indices, graph_pred_path, graph_pred_indices, temp_path, timeout):
    ...
    print("in metrics.stats.novelity before graph_pred_indices")
    graph_pred_indices = write_graphs_from_dir(graph_pred_path, graph_pred_indices, pred_path)
    #graph_pred_indices = write_graphs_from_dir(graph_pred_path, graph_pred_indices, pred_path,len_graphs,subdir_size=len_graphs)
    #graph_ref_indices = write_graphs_from_dir(graph_ref_path, graph_ref_indices, test_path)
    #graph_ref_indices = write_graphs_from_dir(graph_ref_path, graph_ref_indices, test_path,len_graphs,subdir_size)
    #graph_ref_indices =write_graphs_from_dir_with_subdirs(graph_ref_path, graph_ref_indices, test_path,len_graphs,subdir_size)
    print("in metrics.stats.novelity before graph_ref_indices")
    graph_ref_indices =write_graphs_from_dir(graph_ref_path, graph_ref_indices, test_path)
    print("in metrics.stats.novelity after graph_ref_indices")
...
#def uniqueness(graph_pred_path, graph_pred_indices, temp_path,len_graphs,subdir_size,timeout):
    pred_fd, pred_path = tempfile.mkstemp(dir=temp_path)
    res_fd, res_path = tempfile.mkstemp(dir=temp_path)

    write_graphs_from_dir(graph_pred_path, graph_pred_indices, pred_path)
    #write_graphs_from_dir(graph_pred_path, graph_pred_indices, pred_path,len_graphs,subdir_size=len_graphs)
